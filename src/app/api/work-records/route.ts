  ...
  try {
    ...
    if (hasRateOverride) {
      ...
      createNotifications({
        ...
        message: `Work record for ${empName} (${monthStr}) has rate(s) changed from default. Approve to apply entered price. Until then, default price is used.`,
        link: `/work-orders?recordId=${record._id}`,
        metadata: { entityId: String(record._id), entityType: 'work_record', employeeId, employeeName: empName, month: monthStr, amount: finalTotal },
      }).catch(() => {});
    }
